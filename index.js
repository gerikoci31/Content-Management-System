...
async function addRole() {
  ...
  const questions = [
    ...
    },
    {
      ...
      choices: departmentChoices.rows.map(dept => ({
        name: dept.name,
        value: dept.id,
      })),
    },
...
async function addEmployee() {
  ...
  const roleChoices = await pool.query('SELECT * FROM role');
  const managerChoices = await pool.query('SELECT * FROM employee');
  ...
  const questions = [
    ...
    },
    {
      ...
      choices: roleChoices.rows.map(role => ({
        name: role.title,
        value: role.id,
      })),
    },
    {
      ...
      choices: [{ name: 'None', value: null }].concat(managerChoices.rows.map(manager => ({
